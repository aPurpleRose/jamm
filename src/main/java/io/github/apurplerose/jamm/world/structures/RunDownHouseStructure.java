...
public class RunDownHouseStructure extends StructureFeature<StructurePoolFeatureConfig> {
        ...
        private static boolean isFeatureChunk(StructureGeneratorFactory.Context<StructurePoolFeatureConfig> context) {
                ...
                int landHeight = context.chunkGenerator()
                        .getHeightInGround(spawnXZPosition.getX(), spawnXZPosition.getZ(), Heightmap.Type.WORLD_SURFACE_WG, context.world());

                VerticalBlockSample columnOfBlocks = context.chunkGenerator()
                        .getColumnSample(spawnXZPosition.getX(), spawnXZPosition.getZ(), context.world());
        ...
        public static Optional<StructurePiecesGenerator<StructurePoolFeatureConfig>>
        createPiecesGenerator(StructureGeneratorFactory.Context<StructurePoolFeatureConfig> context) {
                ...
                StructurePoolFeatureConfig newConfig = new StructurePoolFeatureConfig(
                        () -> context.registryManager().get(Registry.STRUCTURE_POOL_KEY)
                                .get(new Identifier(Jamm.MOD_ID, "run_down_house/start_pool")), 10
                );
                ...
                Optional<StructurePiecesGenerator<StructurePoolFeatureConfig>> structurePiecesGenerator =
                        StructurePoolBasedGenerator.generate(
                                ...
                                pos,
                                false,
                                true //false in nether
                        );

                if(structurePiecesGenerator.isPresent()) {
                        Jamm.LOGGER.log(Level.DEBUG, "Rundown House at " + pos);
                }
